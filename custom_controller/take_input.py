...
class MoveToNode(Node):
    def __init__(self):
        super().__init__("take_input")
        self.get_logger().info("Node Started")
        self.cmd_vel_pub = self.create_publisher(Twist, "/cmd_vel", 10)#get target x coordinate from input
        qos_profile = QoSProfile(depth =10)
        qos_profile.reliability = QoSReliabilityPolicy.BEST_EFFORT #required for Qos
        self.odom_sub = self.create_subscription(PoseStamped, "/vrpn_mocap/Rigid_body_001/pose", self.vrpn_mocap_pose_callback, qos_profile)#create subscriber to get position via motive tracker
